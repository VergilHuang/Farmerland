  ...
  if (!post) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        <h1 className="text-3xl font-bold text-primary dark:text-slate-100 mb-4">
          Article Not Found
        </h1>
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
        <article className="lg:col-span-8">
          <header className="mb-10">
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              {post.tags?.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full bg-primary/10 dark:bg-slate-700 px-3 py-1 text-xs font-bold uppercase tracking-wider text-primary dark:text-slate-300"
                >
                  {tag}
                </span>
              ))}
          ...
          <div
            className="prose prose-lg dark:prose-invert max-w-none prose-a:text-accent hover:prose-a:text-accent/80 prose-headings:text-primary dark:prose-headings:text-slate-100"
          ...
          <footer className="mt-16 border-t border-primary/10 dark:border-white/10 pt-10">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold uppercase tracking-widest text-primary/40 dark:text-slate-500">
                  Tags:
                </span>
                <div className="flex flex-wrap gap-2">
                  {post.tags?.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-lg bg-primary/5 dark:bg-slate-800 px-3 py-1 text-sm font-medium text-primary/80 dark:text-slate-300"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="flex items-center gap-4">
                <span className="text-sm text-primary/60 dark:text-slate-400">
                  Was this helpful?
                </span>
        ...
        <aside className="hidden lg:block lg:col-span-4">
          <div className="sticky top-28 space-y-8">
            <div className="rounded-xl border border-primary/10 dark:border-white/10 bg-white/50 dark:bg-slate-800/50 p-6 backdrop-blur-sm shadow-sm">
              <h3 className="mb-4 text-sm font-bold uppercase tracking-widest text-primary/40 dark:text-slate-500">
                Table of Contents
              </h3>
              <nav className="flex flex-col gap-1 text-sm">
                <span className="text-primary/60 dark:text-slate-400 italic">
